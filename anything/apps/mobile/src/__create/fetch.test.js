describe("updatedFetch", () => {
  ...
  it("prefers the app URL when it matches the configured host", async () => {
    process.env.EXPO_PUBLIC_BASE_URL = "https://risingabovetrauma.onrender.com";
    process.env.EXPO_PUBLIC_APP_URL =
      "https://02687c08-e3f9-4ef4-9b5e-3fff129ac942.created.app";
    process.env.EXPO_PUBLIC_HOST =
      "02687c08-e3f9-4ef4-9b5e-3fff129ac942.created.app";

    await updatedFetch("/api/auth/mobile-signin");

    expect(expoFetchMock).toHaveBeenCalledWith(
      "https://02687c08-e3f9-4ef4-9b5e-3fff129ac942.created.app/api/auth/mobile-signin",
      expect.objectContaining({
        headers: expect.any(Headers),
      }),
    );
    expect(expoFetchMock.mock.calls[0][1].headers.get("host")).toBe(
      "02687c08-e3f9-4ef4-9b5e-3fff129ac942.created.app",
    );
  });
